...
char *test_delete()
{
    // debug("test1: %p test2: %p test3: %p", &test1, &test2, &test3);
    // debug("expect1: %p expect2: %p expect3: %p", &expect1, &expect2, &expect3);
    
    bstring deleted = (bstring)BSTree_delete(map, &test1);
    // debug("deleted: %p", deleted);
