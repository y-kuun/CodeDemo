...
// this is not balanced tree, so much easy

static int default_compare(void *lhs, void *rhs)
{
    return bstrcmp((bstring)lhs, (bstring)rhs);
}
...
static inline void BSTree_setnode(BSTree *map, BSTreeNode *node, void *key, void *data)
{
    ...
    // if key lexicographically bigger go RIGHT
    if(cmp < 0)
    {
        if(node->left)
        {
...
{
    // node is replaced how about its children
    if(node->parent)
    {
...
{
    void *tmp = NULL;
    tmp = b->key; b->key = a->key; a->key = tmp;
    tmp = b->data; b->data = a->data; a->data = tmp;
}
...
static inline BSTreeNode *BSTree_node_delete(BSTree *map, BSTreeNode *node, void *key)
{
    ...
    {
        // found node needs to be destroy
        if(node->left && node->right)
        {
            // the smallest node in the right tree can have a right child which is bigger than itself
